...
PRED_ROOT=$ROOT"coder/predictions/coder_cocondenser_sep/zeroshot/" #"cocondenser/predictions_zeroshot_sep/" #"coder/predictions/coder_cocondenser_sep/zeroshot/" #"coder/predictions/coder_repbert/trained_on_HEADnTORSO/" # "repbert/predictions/repeat_2/"
PRED_PATH=$PRED_ROOT$MODEL_ALIAS".top1000"
RECORDS_PATH="/gpfs/data/ceickhof/gzerveas/RecipNN/smooth_labels/evaluation_records/TripClick/zeroshot/"  #TripClick/trained_on_HEADnTORSO/"
...
for part in "head" "torso" "tail"; do
  for split in "val" "test"; do
    ...
    if [ "$part" == "head" ]; then
      python ~/Code/multidocscoring_transformer/trec_eval_all.py --pred_path "$PRED_PATH"."$part"."$split".tsv --qrels_path "$QRELS_PATH"dctr."$part"."$split".tsv --relevance_level $RELEVANCE_LEVEL --write_to_json --records_file $RECORDS_PATH"records_DCTR_""$part"."$split".xls
    fi
