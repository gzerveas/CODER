...
QUERIES_DIR=$ROOT"bertbase_reps_dot/processed/"  #"repbert/preprocessed/"

DOC_EMB_DIR=$ROOT"tasb/representations/zeroshot/doc_embeddings_memmap/"
MODEL_ALIAS="CODER_TASB_IZc_zeroshot"
...
for part in "head" "torso" "tail"; do
  for split in "val" "test"; do
    # Embed queries
    python ~/Code/multidocscoring_transformer/precompute.py --model_type mdstransformer --encoder_from "sebastian-hofstaetter/distilbert-dot-tas_b-b256-msmarco" --load_checkpoint $MODEL_PATH --tokenized_queries "$QUERIES_DIR"topics."$part"."$split".json  --output_dir $QUERY_EMB_DIR --aggregation first --per_gpu_batch_size 512
    # Retrieve
